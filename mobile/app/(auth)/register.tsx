...
export default function RegisterStartScreen() {
  ...
  const { role, setPendingAuth } = useAuthStore();
  ...
  const handleRegister = async () => {
    ...
    if (cleanPhone.length !== 10 || password.length < 8) {
    ...
    try {
      ...
      // 2. Trigger Supabase Phone OTP (Bypassed for mock verification)
      console.log(`[Register] Bypassing Supabase OTP trigger for phone: ${fullPhone}`);
      ...
      Toast.show({ type: 'success', text1: 'OTP Sent successfully (Mock)', text2: 'Please use mock code 123456.' });
    ...
    >
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Sign up as a {role?.toLowerCase()} to get started
        </Text>
