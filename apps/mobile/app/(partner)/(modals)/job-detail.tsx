    ...
    <View style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.content}>
          ...
          {/* Job Photos from Client */}
          {job.jobPhotos && job.jobPhotos.length > 0 && (
            <View style={styles.photosSection}>
              <Text style={styles.sectionTitle}>📷 Photos from Client</Text>
              <ScrollView
                horizontal
                showsHorizontalScrollIndicator={false}
                contentContainerStyle={styles.photosRow}
              >
                {job.jobPhotos.map((uri: string, idx: number) => (
                  <Image
                    key={idx}
                    source={{ uri }}
          ...
          {/* Address Contact — tap to call */}
          {job.contactPhone && (
            <View style={styles.contactCard}>
              <View style={styles.contactInfo}>
                <Text style={styles.contactLabel}>📞 Contact at Job Site</Text>
                <Text style={styles.contactName}>{job.contactName || job.client?.name || 'Client'}</Text>
                <Text style={styles.contactNumber}>{job.contactPhone}</Text>
              </View>
              <TouchableOpacity
