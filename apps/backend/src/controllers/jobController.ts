...
export const createJob = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { category, description, address, scheduledDate, workers, rateType, rate, lat, lng, femaleOnly, seasonLabel, materialsIncluded, materialCost, contactName, contactPhone } = req.body;

    // Coerce all fields — they arrive as strings when sent via multipart/form-data
    const parsedLat = parseFloat(lat);
    const parsedLng = parseFloat(lng);
    const parsedRate = parseFloat(rate);
    const parsedWorkers = parseInt(workers, 10) || 1;
    const parsedMaterialCost = materialCost !== undefined && materialCost !== '' ? parseFloat(materialCost) : null;
    const parsedFemaleOnly = femaleOnly === true || femaleOnly === 'true';
    const parsedMaterialsIncluded = materialsIncluded === true || materialsIncluded === 'true';
    ...
    // Create the job first to get an ID for photo storage paths
    const newJob = await prisma.job.create({
      data: {
        ...
        rate: isNaN(parsedRate) ? (rateType === 'DAILY' ? 1200 : 200) : parsedRate,
        scheduledDate: new Date(scheduledDate),
        workerCount: parsedWorkers,
        femaleOnly: parsedFemaleOnly,
        seasonLabel: seasonLabel || null,
        materialsIncluded: parsedMaterialsIncluded,
    ...
    // Upload any client-provided job photos to Firebase Storage
    const files = req.files as Express.Multer.File[] | undefined;
    console.log(`[createJob] files received: ${files?.length ?? 0}`);
    if (files && files.length > 0) {
      const jobPhotoUrls: string[] = [];
      for (const file of files) {
        try {
          const fileName = `job-photos/${newJob.id}/${uuidv4()}-${file.originalname}`;
          const fileUpload = bucket.file(fileName);
          await fileUpload.save(file.buffer, { metadata: { contentType: file.mimetype } });
          await fileUpload.makePublic();
          jobPhotoUrls.push(`https://storage.googleapis.com/${bucket.name}/${fileName}`);
        } catch (fbError) {
          console.error('[createJob] Firebase photo upload failed:', fbError);
        }
      }
      if (jobPhotoUrls.length > 0) {
        await prisma.job.update({
          where: { id: newJob.id },
          data: { jobPhotos: jobPhotoUrls }
        });
        (newJob as any).jobPhotos = jobPhotoUrls;
      }
    }
    ...
    if (nearbyPartnerIds.length > 0) {
      ...
      if (parsedFemaleOnly) whereClause.gender = 'FEMALE';
